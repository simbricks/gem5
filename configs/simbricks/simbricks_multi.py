...
def makeX86System(mem_mode, numCPUs=1, mdesc=None, workload=None, Ruby=False):
    ...
    self.pc.com_1.device = TerminalBuffered(
        port=args.termport, outfile="stdoutput", prefix=args.termprefix)
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="prefix for terminal output",
    )
    parser.add_argument(
        "--simbricks-pci",
        action="append",
...
# First split up command line args into groups at `---` delimiter. Each group
# starts with argv[0] and then has the arguments before the delimiter
arg_groups = []
start = 1
while start < len(sys.argv):
    try:
        end = sys.argv.index('---', start)
    except ValueError:
        end = len(sys.argv)
    arg_groups.append(sys.argv[start:end])
    start = end + 1

# Create single root element
root = Root(full_system=True)

# Create a system for each arg group
i = 0
for args in arg_groups:
    ...
    np = args.num_cpus
    system = build_system(np)

    Simulation.setWorkCountOptions(system, args)

    # Set system as attribute on root object
    setattr(root, f'sys{i}', system)
    i += 1
...
Simulation.run(args, root, system, FutureClass, parametrize_cpus=param_cpus)
