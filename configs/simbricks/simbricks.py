...
def connectX86ClassicSystem(x86_sys, numCPUs):
    ...
    #  (The upper 64kB are reserved for m5ops.)
    #  2) the bridge to pass through the IO APIC
    # (two pages, already contained in 1),
...
def makeLinuxX86System(mem_mode, numCPUs=1, mdesc=None, Ruby=False,\
    ...
    self = makeX86System(mem_mode, numCPUs, mdesc, X86FsLinux(), \
        Ruby, noSimbricks)
    ...
    # Command line
    if not cmdline:
        cmdline = 'earlyprintk=ttyS0 console=ttyS0 root=/dev/sda1 \
            no_timer_check memory_corruption_check=0 \
                random.trust_cpu=on init=/home/ubuntu/guestinit.sh'
...
def build_system(np):
    ...
    if options.ruby:
        ...
        if options.simpoint_profile:
            ...
            if np > 1:
                fatal("SimPoint generation not supported \
                    with more than one CPUs")
...
parser.add_option("--simbricks-pci", action="store", type="string",
        default="/tmp/simbricks-pci", \
            help="Simbricks PCI Unix socket")
parser.add_option("--simbricks-shm", action="store", type="string",
        default="/dev/shm/dummy_nic_shm", \
            help="Simbricks shared memory region")
